...
var root = data;
  ...
  var circle = g.selectAll("circle")
    ...
    .enter().append("circle")
      ...
      .on("click", function(d) { if (focus !== d) zoom(d),selectDimensionValue(d.depth,d.data.name), d3.event.stopPropagation(); });
...
function selectDimensionValue(dimIndex, Value){
if (dimIndex==-1){
	return layoutColor;
}
else	
...
app.field("["+Dimensions[dimIndex-1].qFallbackTitle+"]").selectValues([Value],true,true)
var arr = [];
arr.push(Value);
console.log(Number(parseInt(dimIndex-1,10)));
